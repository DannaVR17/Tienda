...
const {
  ...
  updateShirt,
  deleteShirt
} = require('../Controllers/jugueteController');
...
async function routeRequest(req, res) {
  ...
  // GET /juguetes
  if (method === 'GET' && pathname === '/juguetes') {
  ...
  if (method === 'GET' && pathname.startsWith('/juguete/')) {
    ...
    if (!shirt) {
      ...
      return res.end(JSON.stringify({ error: 'juguete no encontrada.' }));
    }else{
      res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  if (method === 'DELETE' && pathname.startsWith('/juguete/')) {
    ...
    try {
      ...
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ mensaje: 'juguete eliminada correctamente.' }));
  
    } catch (err) {
      console.error(err);
      res.writeHead(500, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'Error al eliminar la juguete.' }));
    }
  }
